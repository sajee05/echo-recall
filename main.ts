...
class RevisionItemView extends ItemView {
    ...
    originalTexts: WeakMap<Text, string> = new WeakMap();
    ...
    applyMaskToDOM() {
        ...
        while ((node = walker.nextNode() as Text)) {
            ...
            if (s.skipCallouts && el.closest('.callout')) { node.nodeValue = original; continue; }
            if (s.skipCheckboxes && el.closest('.task-list-item')) { node.nodeValue = original; continue; }
            if (s.skipQuotes && el.closest('blockquote:not(.callout)')) { node.nodeValue = original; continue; }
            if (s.excludeInternalLinks && el.closest('.internal-link')) { node.nodeValue = original; continue; }
            if (s.excludeExternalLinks && el.closest('.external-link')) { node.nodeValue = original; continue; }
            if (s.excludeEmbeds && el.closest('.internal-embed')) { node.nodeValue = original; continue; }
